...
subroutine compute_chi(lam,ik,ns,xc,lbes4)
  ...
  if (tm) then
     do n=1,ik
        ...
        chis(n,ns) = (enls(ns) + (2*lam+2)/grid%r(n)*dpoly + &
             d2pr(xc,xc(7),grid%r(n)) + dpoly**2 - vpsloc(n))*phis(n,ns)
     enddo
     do n = ik+1,grid%mesh
  ...
  nstart=5
  do n=1,nstart-1
     chis(n,ns)= (g0+grid%r(n)*(g1+g2*grid%r(n)))*grid%r(n)**(lam+3)/grid%sqr(n)
  enddo
  do n=1,grid%mesh
  ...
  !
  do n=1,grid%mesh
     phis(n,ns)=phis(n,ns)/grid%sqr(n)
  enddo
  do n=1,grid%mesh
     j1(n)=grid%r2(n)*(vpsloc(n)-enls(ns))+(lam+0.5_dp)**2
  ...
  chis(grid%mesh-2,ns)=0.0_dp
  chis(grid%mesh-1,ns)=0.0_dp
  chis(grid%mesh,ns)=0.0_dp
  !
  !    and solve it
  !
  call tridiag(arow(nstart),brow(nstart),crow(nstart), &
       drow(nstart),chis(nstart,ns),grid%mesh-3-nstart)
  !
  !   put the correct normalization and r dependence
  !  
  do n=1,grid%mesh
     phis(n,ns)=phis(n,ns)*grid%sqr(n)
     chis(n,ns)=chis(n,ns)*grid%sqr(n)/grid%r2(n)
     !         if(lam.eq.0)
     !     +    write(*,'(5(e20.13,1x))')
  ...
  do n=nstart,grid%mesh
     if (abs(chis(n,ns)/grid%r(n)**(lam+1)-aux(n))  &
          .lt.1.e-3_dp*abs(aux(n)) ) goto 100
     chis(n,ns)=aux(n)*grid%r(n)**(lam+1)
  enddo

100 if (n.eq.grid%mesh+1.or.grid%r(n).gt.0.05_dp)then
     print*,lam,ns,n,grid%mesh,grid%r(n)
