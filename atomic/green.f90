...
!---------------------------------------------------------------
   ...
   real(DP) :: fac
   !      parameter(ndmx=700)
   !      common/radmes/ xmin,zmesh,dx,r(ndmx),r2(ndmx),sqr(ndmx),rmax,mesh
   !     +  /potentials/ vpot(ndmx),vnew(ndmx),vh(ndmx),vx(ndmx),vold(ndmx)
...
! wave-function in the first two points by series developement
   ...
   rr1=(c0+grid%r(1)*(c1+grid%r(1)*(c2+grid%r(1)*(c3+grid%r(1)*c4))))*grid%r(1)**l1
   rr2=(c0+grid%r(2)*(c1+grid%r(2)*(c2+grid%r(2)*(c3+grid%r(2)*c4))))*grid%r(2)**l1
   y(1)=rr1/grid%sqr(1)
   y(2)=rr2/grid%sqr(2)
!
